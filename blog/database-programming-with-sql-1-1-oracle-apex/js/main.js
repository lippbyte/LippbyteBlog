/* ══════════════════════════════════════
   Lippbyte Blog — Main Script
   Handles: theme toggle, progress bar,
   sidebar nav tracking + progress rings
══════════════════════════════════════ */
...
const NAV_H     = 60;
const CIRCUMFERENCE = 2 * Math.PI * 15.5; // ~97.4 for r=15.5
...
function getSectionProgress(section) {
  ...
  // How much of this section has been scrolled past
  const scrolledPast = (NAV_H + 40) - rect.top;

  if (scrolledPast <= 0) return 0;
  if (scrolledPast >= sectionHeight) return 100;

  return Math.round((scrolledPast / (sectionHeight - viewportHeight * 0.3)) * 100);
}
...
function updateNav() {
  ...
  // Update progress rings for each section
  ...
  // Update toc-sub active states
  const allH2s = document.querySelectorAll('.content-body h2[id]');
  let activeH2 = null;
  allH2s.forEach(h2 => {
    if (h2.getBoundingClientRect().top <= NAV_H + 60) activeH2 = h2;
  });
